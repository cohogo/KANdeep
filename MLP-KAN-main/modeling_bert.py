...
class MoE_KAN_MLP(nn.Module):
    ...
    def __init__(self, ffn_dim, hidden_dim, num_experts, top_k):
        ...
        self.experts = nn.ModuleList(
            [Mlp(in_features=self.hidden_dim, hidden_features=self.ffn_dim) for _ in range(self.num_experts // 2)])

        for _ in range(self.num_experts // 2):
            kan = KAN([self.hidden_dim, self.ffn_dim, self.hidden_dim])
...
class kanBlock():
    def __init__(self, dim, hdim_kan=192, norm_layer=nn.LayerNorm):
        super().__init__(dim)
    ...
    def forward(self, x):
        b, t, d = x.shape
        x = self.kan(self.norm2(x).reshape(-1, x.shape[-1])).reshape(b, t, d)
        return x
